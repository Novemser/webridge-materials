...
/**
 * utility class to find all files matching (possibly hierarchical) wildcard path specs
 *
 * <p>we support single '*' wildcards as in filename matching, plus "**" patterns that match all
...
public class FileUtils {
  ...
  /**
   * turn a mixed path list into a valid Unix path set without drive letters, and with '/' and ':'
   * separators. Also remove multiple consecutive separators this assumes the path String to be
   * already expanded
   */
  ...
  /**
   * turn a mixed path list into a valid Windows path set with drive letters, and '\' and ';'
   * separators. Also remove multiple consecutive separators this assumes the path String to be
   * already expanded
   */
  ...
  public static boolean ensureDirs(File file, boolean isFileDir) {
    final File dir;
    if (isFileDir) {
      dir = file;
    } else {
