...
import com.webridge.WeBridgeUtils;
...
public class PreparedStatement extends com.mysql.jdbc.StatementImpl implements java.sql.PreparedStatement {
    ...
    public boolean execute() throws SQLException {
        if (com.webridge.Constants.ENABLE_WEBRIDGE &&
                !WeBridgeUtils.isInsideSpManager()) {
            WeBridgeUtils.enterSpManager();
            try {
                return SPInvokeManager.doExecute(this);
            } finally {
                WeBridgeUtils.leaveSpManager();
            }
        }
        synchronized (checkClosed().getConnectionMutex()) {
    ...
    public java.sql.ResultSet executeQuery() throws SQLException {
        if (com.webridge.Constants.ENABLE_WEBRIDGE &&
            ...
            WeBridgeUtils.enterSpManager();
            try {
                ResultSet resultSet = SPInvokeManager.doExecuteQuery(this);
                return resultSet;
            } finally {
                WeBridgeUtils.leaveSpManager();
            }
    ...
    public int executeUpdate() throws SQLException {
        if (com.webridge.Constants.ENABLE_WEBRIDGE && !WeBridgeUtils.isInsideSpManager()) {
            try {
                WeBridgeUtils.enterSpManager();
                int res = SPInvokeManager.doExecuteUpdate(this);
                return res;
            } finally {
                WeBridgeUtils.leaveSpManager();
            }
        ...
        synchronized (checkClosed().getConnectionMutex()) {
            ...
            if (this.pstmtResultMetaData == null) {
                try {
                    ...
                    if (paramCount >= 2 && this.originalSql.trim().toUpperCase().endsWith("LIMIT ?,?")) {
                        // for SQL statements like: select xxx where id = ? LIMIT ?,? we
                        // only set the params excluding ones in the limit expression
                        // to avoid syntax errors
                        mdStmt.setLong(paramCount - 1, 0);
                        mdStmt.setLong(paramCount, 1);
                        for (int i = 1; i <= paramCount - 2; i++) {
                            mdStmt.setString(i, "");
                        }
                    } else if (paramCount >= 1 && this.originalSql.trim().toUpperCase().endsWith("LIMIT ?")) {
                        mdStmt.setLong(paramCount, 1);
    ...
    public int getUpdateCount() throws SQLException {
        if (com.webridge.Constants.ENABLE_WEBRIDGE && !WeBridgeUtils.isInsideSpManager()) {
            try {
                WeBridgeUtils.enterSpManager();
                int res = SPInvokeManager.doExecuteUpdate(this, true);
                return res;
            } finally {
                WeBridgeUtils.leaveSpManager();
            }
