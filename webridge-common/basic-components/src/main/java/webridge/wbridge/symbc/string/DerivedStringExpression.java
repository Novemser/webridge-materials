...
import webridge.wbridge.symbc.numeric.ConstraintExpressionVisitor;
...
public class DerivedStringExpression extends StringExpression {
    ...
    switch (op) {
      ...
      default:
        throw new RuntimeException(
            "## Error: BinaryStringSolution solution: l " + l + " op " + op + " r " + r);
    }
  }
  ...
  public String stringPC() {
    if (left != null) return left.stringPC() + "." + op.toString() + "(" + right.stringPC() + ")";
    else if (right != null) return "." + op.toString() + "(" + right.stringPC() + ")";
    else {
  ...
  public String toString() {
    if (left != null) return left.toString() + "." + op.toString() + "(" + right.toString() + ")";
    else if (right != null) return "." + op.toString() + "(" + right.toString() + ")";
    else {
      StringBuilder s = new StringBuilder();
      s.append("[");
      for (int i = 0; i < oprlist.length; i++) {
  ...
  public String getName() {
    String name;
    if (left != null) name = left.getName() + "_" + op.toString() + "__" + right.getName() + "___";
    else if (right != null) name = "_" + op.toString() + "__" + right.getName() + "___";
    else {
      StringBuilder s = new StringBuilder();
      s.append("__");
      for (int i = 0; i < oprlist.length; i++) {
  ...
  public boolean equals(Object o) {
    ...
    DerivedStringExpression that = (DerivedStringExpression) o;
    return Objects.equals(left, that.left)
        && op == that.op
        && Objects.equals(right, that.right)
        && Arrays.equals(oprlist, that.oprlist);
  }

  @Override
  public int hashCode() {
    int result = Objects.hash(left, op, right);
    result = 31 * result + Arrays.hashCode(oprlist);
    return result;
  }
}
