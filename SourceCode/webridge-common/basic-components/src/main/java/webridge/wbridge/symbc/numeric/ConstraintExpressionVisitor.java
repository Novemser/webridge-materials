...
import webridge.wbridge.symbc.concolic.FunctionExpression;
...
import webridge.wbridge.symbc.string.DerivedStringExpression;
import webridge.wbridge.symbc.string.StringConstant;
import webridge.wbridge.symbc.string.StringConstraint;
import webridge.wbridge.symbc.string.StringExpression;
import webridge.wbridge.symbc.string.StringSymbolic;
import webridge.wbridge.symbc.string.SymbolicStringBuilder;

/**
 * A visitor for both constraints and expressions. Ideally, constraints should be an extension of
 * expressions themselves, but, alas!, they are not.
 *
 * <p>Each node N in the expression tree (which may be a DAG) has an operator, zero or more operands
 * O1, O2, O3, ..., and implements a routine called "accept(ConstraintExpressionVisitor visitor)"
 * which calls the methods of this visitor in the following way:
 *
 * <ol>
 *   <li>When node N is encountered, visitor.preVisit(N).
 *   <li>Before the first operand O1 is visited, visitor.inVisitBefore(N, O1) is called.
 *   <li>O1 is visited.
 *   <li>visitor.inVisitAfter(N, O1) is called.
 *   <li>Before the second operand O2 is visited, visitor.inVisitBefore(N, O2) is called.
 *   <li>O2 is visited.
 *   <li>visitor.inVisitAfter(N, O2) is called.
 *   <li>...
 *   <li>Before the operand Ok is visited, visitor.inVisitBefore(N, Ok) is called.
 *   <li>Ok is visited.
 *   <li>visitor.inVisitAfter(N, Ok) is called.
 *   <li>Finally, once all the operands have been visited, visitor.postVisit(N) is called.
 * </ol>
 *
 * For the moment, visitor.inVisitBefore(...) and visitor.inVisitAfter(...) routines are not
 * implemented. They would add another 1134 methods to this class, and although it would not impact
 * runtime efficiency, it does seem like overkill.
 *
...
public abstract class ConstraintExpressionVisitor {
  ...
  public void preVisit(GeneralMixedConstraint constraint) {}
  ...
  public void postVisit(GeneralMixedConstraint constraint) {}
