...
import static webridge.wbridge.sql.SQLFormatter.formatSQL;

import webridge.wbridge.sql.result.MultipleRowResultSetImpl;
...
public class ResultSetHolder {
  ...
  public static ResultSet executeQuery(String requiredTemplate) {
    ...
    if (!formatSQL(requiredTemplate).equalsIgnoreCase(formatSQL(recordedTemplate))) {
      throw new RuntimeException(
              String.format(
                      "SQL mismatch! This means that we have executed a wrong path (caused by global variable fork, etc). \nRequired:%s\nRecorded:%s",
                      formatSQL(requiredTemplate), formatSQL(recordedTemplate)));
    }
    //    System.out.println(
  ...
  public static int executeUpdate(String requiredTemplate) {
    ...
    if (!formatSQL(requiredTemplate).equals(formatSQL(recordedTemplate))) {
  ...
  public static void doTxnRelatedSQL(String requiredSql) {
    ...
    if (!formatSQL(requiredSql).equals(formatSQL(recordedSql))) {
      throw new RuntimeException(
              String.format(
                      "SQL mismatch! This means that we have executed a wrong path (caused by global variable fork, etc). \nRequired:%s\nRecorded:%s",
                      formatSQL(requiredSql), formatSQL(recordedSql)));
    }
