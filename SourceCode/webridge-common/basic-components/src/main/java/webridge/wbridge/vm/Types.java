 ...
 *
 * <p>This reflects the general type id mess in Java. We support the following:
 *
 * <p>builtin type: byte - T_BOOLEAN and the like type name: String - according to JLS 6.7 ("int",
 * "x.Y[]") type signature: String - like JNI ("I", "[Lx/Y;") type classname: String - e.g. "int",
 * "[I", "x.Y", "[Lx.Y;"
 */
public class Types {
  ...
  private static final Map<String, String> typeNameClassNameMapping = new HashMap<>();
  ...
  /**
   * get the argument type part of the signature out of a JNI mangled method name. Note this is not
   * the complete signature, since we don't have a return type (which is superfluous since it's not
   * overloading, but unfortunately part of the signature in the class file)
   */
  ...
  /**
   * return the name part of a JNI mangled method name (which is of course not completely safe - you
   * should only use it if you know this is a JNI name)
   */
   ...
   *
   * <p>int -> int int[] -> [I String -> java/lang/String String[] -> [Ljava/lang/String; String[][]
   * -> [[Ljava/lang/String;
   *
   * <p><2do> this is really not very efficient
   */
    ...
    if (signature.charAt(signature.length() - 1) == ';') { // reference
      ...
      switch (signature.charAt(0)) {
        ...
        case 'D':
          return "double";
        default:
          throw new RuntimeException("illegal type signature: " + signature);
      }
    }
  }

  /**
   * get the canonical representation of a type name, which happens to be (1) the name of the
   * builtin type (e.g. "int") (2) the normal dot name for ordinary classes (e.g.
   * "java.lang.String") (3) the coded dot name for arrays (e.g. "[B", "[[C", or
   * "[Ljava.lang.String;")
   *
   * <p>not sure if we need to support internal class names (which use '/' instead of '.' as
   * separators
   *
   * <p>no idea what's the logic behind this, but let's implement it
   */
  public static String getClassNameFromTypeName(String typeName) {
    // this method is computation intensive, should optimize with care
    // currently the implementation is not thread safe
    final String res = typeNameClassNameMapping.get(typeName);
    if (res != null) return res;
    final String resolvedName = doClassNameFromTypeNameInternal(typeName);
    typeNameClassNameMapping.put(typeName, resolvedName);
    return resolvedName;
  }

  public static String doClassNameFromTypeNameInternal(String typeName) {
  ...
  public static byte getTypeCode(String signature) {
    ...
    switch (c) {
      ...
      default:
        throw new RuntimeException("unknow typecode: " + signature);
    }
  }
  ...
  public static int getTypeLength(String signature, int idx) {
    switch (signature.charAt(idx)) {
      ...
      case 'T':
        ...
        int leftAngleBracket = signature.indexOf('<', idx);
        int rightAngleBracket = signature.indexOf('>', idx);
        boolean isGeneric = leftAngleBracket < semicolon && leftAngleBracket != -1;
        ...
        if (!isGeneric) {
          ...
          return rightAngleBracket - idx + 2;
        }
    }
  ...
  /**
   * return the JPF internal representation of a method signature that is given in dot-notation
   * (like javap),
   *
   * <p>e.g. "int foo(int[],java.lang.String)" -> "foo([ILjava/lang/String;)I"
   */
  public static String getSignatureName(String methodDecl) {
    ...
    throw new RuntimeException("invalid method declaration: " + methodDecl);
  }
}
